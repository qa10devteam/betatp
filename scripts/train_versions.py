...
def build_h2h(df):
    ...
    FIXED Bug #4: obie kolumny _w/_l liczone z perspektywy winner_id i loser_id osobno,
    NIE z perspektywy 'kto wygrał ten mecz' — inaczej h2h_surf_winrate_w = zawsze winrate
    gracza który wygrał = leakage.
    """
    df = df.sort_values("tourney_date").reset_index(drop=True)
    h2h = {}  # (p1,p2) → list of (date, winner_id, surface)

    h2h_delta_w = []  # delta H2H last-3 z perspektywy winner_id (pre-match)
    h2h_delta_l = []  # delta H2H last-3 z perspektywy loser_id (pre-match)
    h2h_surf_wr_w = []  # surface winrate winner_id (pre-match)
    h2h_surf_wr_l = []  # surface winrate loser_id (pre-match)
    ...
    for _, row in df.iterrows():
        ...
        # H2H delta — z perspektywy każdego gracza niezależnie
        wins_w = sum(1 for (_, winner, _) in last3 if winner == wid)
        wins_l = sum(1 for (_, winner, _) in last3 if winner == lid)
        h2h_delta_w.append(wins_w - (len(last3) - wins_w))
        h2h_delta_l.append(wins_l - (len(last3) - wins_l))

        # Surface winrate — z perspektywy każdego gracza niezależnie
    ...
    df["h2h_wins_delta_3"] = h2h_delta_w
    df["h2h_surf_winrate"]  = h2h_surf_wr_w
    return df
...
def run_v17(df):
    ...
    pairs = fix_base_pairs(df2)+WEATHER_PAIRS+[
        ("h2h_wins_delta_3_w", "h2h_wins_delta_3_l"),("rank_traj_w", "rank_traj_l"),
...
def run_v20(df):
    ...
    meta={"version":"v20","mean_holdout_auc":round(mean_auc,4),
          "holdout_auc":round(mean_auc,4),"holdout_bs":round(np.mean([b for _,(m,a,b) in surf_models.items()]),4),
